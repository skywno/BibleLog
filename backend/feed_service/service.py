...
class FeedService:
    def __init__(
        ...
        event_bus.subscribe("ReactionToggled", lambda _: self._cache.invalidate_all_feeds())
        event_bus.subscribe("CommentCreated", lambda _: self._cache.invalidate_all_feeds())
    ...
    def get_feed(
        ...
        entries = self._load_timeline(viewer_id, feed_filter, sort)
        ...
        summaries = self._notes.batch_summaries(viewer_id, note_ids)
        summary_by_id = {summary.id: summary for summary in summaries}
        reactions = self._social.get_reactions_batch(note_ids, viewer_id)
        comment_counts = self._social.get_comment_counts_batch(note_ids)
    ...
    def toggle_reaction(self, viewer_id: str, note_id: str, reaction) -> FeedItem:
        note = self._notes.get_detail(viewer_id, note_id)
        self._social.toggle_reaction(note_id, viewer_id, reaction)
        reaction_rows = self._social.get_reactions_batch([note_id], viewer_id).get(note_id, [])
        comment_count = self._social.get_comment_counts_batch([note_id]).get(note_id, 0)
        return FeedItem(
            note=note,
            reactions=[
    ...
    def _load_timeline(
        ...
        if cached:
            return self._entries_from_cache(cached)

        entries = self._assemble_entries(viewer_id, feed_filter)
        if sort == "popular":
            scored = []
            for entry in entries:
                reaction_total, comment_count = self._social.total_engagement(entry.note_id)
    ...
    def _entries_from_cache(self, cached: list[tuple[str, float]]) -> list[FeedTimelineEntry]:
        entries: list[FeedTimelineEntry] = []
        for note_id, _score in cached:
            entry = self._notes.get_timeline_entry(note_id)
            if entry is not None:
                entries.append(entry)
        return sort_entries(entries)

    def _assemble_entries(self, viewer_id: str, feed_filter: FeedFilter) -> list[FeedTimelineEntry]:
        author_ids = self._collect_author_ids(viewer_id, feed_filter)
        author_ids = sorted(set(author_ids) - {viewer_id})
        return self._notes.recent_entries_for_feed(viewer_id, author_ids)

    def _collect_author_ids(self, viewer_id: str, feed_filter: FeedFilter) -> list[str]:
        membership = self._relations.get_membership(viewer_id)
        friend_ids = self._relations.list_friend_ids(viewer_id)
        ...
        if feed_filter == "small_group":
            if not membership.group_ids:
                return []
            return self._relations.list_group_member_ids(membership.group_ids)

        if feed_filter == "church":
            if membership.church_id is None:
                return []
            return self._relations.list_church_member_ids(membership.church_id)

        group_member_ids: list[str] = []
        if membership.group_ids:
            group_member_ids = self._relations.list_group_member_ids(membership.group_ids)
        church_member_ids: list[str] = []
        if membership.church_id:
            church_member_ids = self._relations.list_church_member_ids(membership.church_id)
