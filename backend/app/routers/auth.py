...
from app.deps import SettingsDep, get_current_user_id
from app.models import AuthTokenResponse, OAuthAuthorizeResponse, RefreshTokenRequest
...
async def get_oauth_authorize_url(
    ...
    store.oauth_states[state] = redirect_uri
...
async def handle_oauth_callback(
    ...
    payload = issue_tokens(user_key, nickname, settings)

    client_redirect = store.oauth_states.pop(state, None) if state else None
    if client_redirect:
        fragment = urlencode(
            {
    ...
    if state and state in store.oauth_states:
        del store.oauth_states[state]
...
def refresh_auth_token(
    ...
    user_id = store.pop_refresh_token(body.refresh_token)
    if not user_id:
...
def dev_login_endpoint(
    ...
    return AuthTokenResponse(**dev_login(email, settings))
